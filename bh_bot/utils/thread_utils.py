...
def thread_function(*, func: Callable[..., None], callback: Callable[[Optional[Exception], Optional[Any]], None], thread_id: str, **kwargs: Any) -> None:
    ...
    def target():
        try:
            ...
            # Runs when abrupt exit occurs (hitting esc). Result will be None initially.
            if stop_event.is_set() and result is None:
                threading.Thread(target=callback, args=(
                    None, STATUS["esc"])).start()
                return
    ...
    if thread_id == "run_all":
        for key in list(stop_events.keys()):
            if key.startswith("run_all"):
                stop_events[key].set()
                tprint(f"Canceling task: {key}")
        stop_sleep()
    elif thread_id in stop_events:
        stop_events[thread_id].set()
        stop_sleep()
        tprint(f"Canceling task: {thread_id}")
    else:
        tprint(f"No task found with thread_id: {thread_id}")
