...
def raid(*, user_settings, user, stop_event: threading.Event, start_time=time.time()):
    ...
    if persuade_button_location is not None:
        ...
        if user_settings["R_auto_bribe"] and get_bribe_list(anchor_location=persuade_button_location, running_window=running_window, username=user["username"]) is True:
