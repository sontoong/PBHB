...
def run_with_retries(*, func, thread_id, user_settings, user, **kwargs):
    num_of_retries = user_settings["D_num_of_loop"]
    delay = 2
...
def thread_dungeon(*, callback, user, user_settings) -> None:
    thread_id = f"{dungeon.__name__}"

    def apply_loop(**kwargs):
        return run_with_retries(func=dungeon, thread_id=thread_id,
