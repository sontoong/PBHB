...
def run_with_retries(*, func, thread_id, user_settings, user, **kwargs):
    num_of_retries = user_settings["WB_num_of_loop"]
    delay = 2
...
def thread_world_boss(*, callback, user, user_settings) -> None:
    thread_id = f"{world_boss.__name__}"

    def apply_loop(**kwargs):
        return run_with_retries(func=world_boss, thread_id=thread_id,
